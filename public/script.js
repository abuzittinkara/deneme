...
let currentRoom = null; // Text veya voice kanalın ID'si
...
function initSocketEvents() {
    ...
    roomsArray.forEach(roomObj => {
      ...
      roomItem.addEventListener('click', () => {
        if (roomObj.type === 'text') {
          ...
          // Güncelle: currentRoom'ı metin kanalının ID'si olarak ata
  ...
  socket.on('textHistory', (messages) => {
    const container = document.getElementById('channelContentArea');
    let historyDiv = container.querySelector('.text-messages-history');
    if (!historyDiv) {
      historyDiv = document.createElement('div');
      historyDiv.className = 'text-messages-history';
      container.prepend(historyDiv);
    }
    historyDiv.innerHTML = '';
    messages.forEach(msg => {
      const msgDiv = document.createElement('div');
      msgDiv.className = 'text-message';
      msgDiv.innerHTML = `<strong>${msg.user.username}:</strong> ${msg.content}`;
  ...
  socket.on('newTextMessage', (data) => {
    ...
    const msgDiv = document.createElement('div');
    msgDiv.className = 'text-message';
    msgDiv.innerHTML = `<strong>${data.message.username}:</strong> ${data.message.content}`;
    historyDiv.appendChild(msgDiv);
  });
}
...
function startSfuFlow() {
  ...
  createTransport().then(transportParams => {
    if (transportParams.error) {
      console.error("createTransport error:", transportParams.error);
      return;
    }
    if (!deviceIsLoaded) {
      device.load({ routerRtpCapabilities: transportParams.routerRtpCapabilities })
        .then(() => {
          deviceIsLoaded = true;
          console.log("Device load bitti =>", device.rtpCapabilities);
          setupSendTransport(transportParams);
        });
    } else {
      setupSendTransport(transportParams);
    }
  });
  function setupSendTransport(transportParams) {
    ...
    createTransport().then(recvParams => {
      if (recvParams.error) {
