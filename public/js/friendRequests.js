export function initFriendRequests(socket) {
  ...
  // Event delegation ile dm-filter-item tıklamalarını yakalıyoruz.
  // selectedDMBar, dmChannelTitle yeniden render edilse bile kalıcı olan ebeveyn konteyner.
  const selectedDMBar = document.getElementById('selectedDMBar');
  if (selectedDMBar) {
    selectedDMBar.addEventListener('click', function(e) {
      const target = e.target;
      if (target.classList.contains('dm-filter-item')) {
               ...
               if (response.friends.length === 0) {
                 ...
                 response.friends.forEach(friend => {
                   ...
                   friendItem.addEventListener('click', () => {
                     ...
                     // selectedDMBar içeriğini güncelle
