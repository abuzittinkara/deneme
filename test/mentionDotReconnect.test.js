...
test('mention dot restored on reconnect', async () => {
  const { socket } = await setup();
  window.selectedGroup = 'g1';
  socket.emit('groupsList', [{ id: 'g1', name: 'G1', owner: 'u1' }]);
  socket.emit('mentionCounts', { g1: { c1: 1 } });
  socket.emit('unreadCounts', { g1: { c1: 1 } });
  socket.emit('roomsList', [{ id: 'c1', name: 'C1', type: 'text', unreadCount: 1 }]);
